...
ggplot() + 
  geom_from_path(aes(.5, .5, path = here(image_path))) +
  theme_void()
```

Time to build the sticker. 

- I used the `hex_crop()` function to crop the image in a hexagonal shape, with a border of a specified color and size. I had to dug a bit to find the exact grey-blue colour of the GitHub website lines to use as the border colour for a cool "merging with the background" effect.
...
ggplot() + 
  geom_from_path(aes(.5, .5, path = here(stick_path))) +
  scale_x_continuous(limits = c(0, 1), expand = c(0, 0)) +
  scale_y_continuous(limits = c(0, 1), expand = c(0, 0)) +
  theme_void()
```
